...
const OtherBrands = () => {
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(order =>
        ...
        (order.brand_name && order.brand_name.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
  ...
  const exportToCSV = () => {
    const headers = ['Customer Name', 'Phone', 'City', 'Brand', 'Product', 'Price', 'Quantity', 'Shipping', 'Total', 'Status', 'Notes'];
              ...
              {filteredOrders.length > 0 ? (
                filteredOrders.map((order) => (
                  <tr key={order.id} className="border-b border-primary-border hover:bg-primary-border/50">
                    ...
                    <td className="py-4 px-6">
                      {order.brand_name ? (
                        <span className="px-2 py-1 bg-primary-blue/20 text-primary-blue rounded text-sm">
                          {order.brand_name}
                        </span>
                      ) : (
